...
def rectangle_mask(image_shape):
    ...
    rotated_coords[:,:,0] = np.cos(angle)*coords[:,:,0] - np.sin(angle)*coords[:,:,1]
    rotated_coords[:,:,1] = np.sin(angle)*coords[:,:,0] + np.cos(angle)*coords[:,:,1]
    mask = np.logical_and(np.abs(rotated_coords[:,:,0]) < dims[0]/2, np.abs(rotated_coords[:,:,1]) < dims[1]/2)
    return mask

def random_walks(image_shape):
    ...
    for i in range(int(np.mean(image_shape))):
        ...
        step_positions = step_positions + step
        step_positions = np.minimum(step_positions, image_shape-1)
        step_positions = np.maximum(step_positions, 0)
        mask[step_positions[:,0], step_positions[:,1]] = True
    return mask
...
def random_ellipse(image_shape):
    ...
    rotated_coords[:,:,0] = np.cos(angle)*coords[:,:,0] - np.sin(angle)*coords[:,:,1]
    rotated_coords[:,:,1] = np.sin(angle)*coords[:,:,0] + np.cos(angle)*coords[:,:,1]
    mask = np.less_equal((np.power(rotated_coords[:,:,0]/dims[0], 2) + np.power(rotated_coords[:,:,1]/dims[1], 2)), 1)
    return mask
def random_n_masks(image_shape, min = 4, max = 7):
    mask = np.zeros(image_shape)
    n = np.random.choice(range(min, max+1))
    for i in range(n):
...
def plot_n_topologies(topologies):
    ...
    for i, sub in enumerate(g):
        ...
        plt.imshow(topologies[i], cmap = "gray")
        #turn ticks off
        ax.set_xticks([])
        ax.set_yticks([])
    fig.tight_layout()
